...
+(void)modificaNumeroPlist:(NSString *)nomeFile conChiave:(NSString*)chiave eNumero:(NSNumber*)numero{
    ...
    //NSString *localizedPath = [[NSBundle mainBundle] pathForResource:nomeFile ofType:@"plist"];
    NSString *pathString = [nomeFile stringByAppendingString:@".plist"];
    NSString *applicationDocumentsDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *storePath = [applicationDocumentsDir stringByAppendingPathComponent:pathString];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSMutableDictionary *tempPlist = nil;
    if ([fileManager fileExistsAtPath: storePath]) {
        tempPlist = [NSMutableDictionary dictionaryWithContentsOfFile:storePath];
        [tempPlist setObject:numero forKey:chiave];
        NSLog(@"Here am I");
        BOOL success = [tempPlist writeToFile:storePath atomically:YES];
        NSLog(@"Here am I again");
        if (success)
            NSLog(@"Success! %d",[[self valuePlist:nomeFile withKey:chiave] intValue]);
    }
    else {
        NSLog(@"NRSimplePlist - Errore nella MODIFICA NUMERO del file plist '%s'", [storePath UTF8String],nil);
        NSLog(@"NRSimplePlist - Error EDITING NUMBER of plist file '%s'", [storePath UTF8String],nil);
    }
}
